...
export const MdcBaseIconButton: (props: (Partial<HTMLElement> & MDCIconButtonProps)) => TemplateResult = virtual(function (this: HTMLElement, {
  icon,
  selected,
  disabled,
  selectable,
  ...
  return html`
      ...
      })} @mouseup=${removeFocus} @click=${handleClick}>
          <button tabindex="0" ?disabled=${disabled} class=${classMap({
              'active': isActive,
          })}>
              ${MdcIcon({
                  name: icon,
